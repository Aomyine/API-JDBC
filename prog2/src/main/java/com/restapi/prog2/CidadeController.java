...
public class CidadeController {
	...
	Cidade createCidade(@RequestBody Cidade p) {
		int maxId = 1;
		for (Cidade prof: Cidades) {
			if (prof.getIdCidade() > maxId) {
				maxId = prof.getIdCidade();
			}
		}
	...
	Optional<Cidade> updateCidade(@RequestBody Cidade CidadeRequest, @PathVariable long CidadeId) {
